...
$RequiredFiles = @(
    "Cards\EdictCard.cs",
    "Cards\ChuanLingCard.cs",
    "Cards\MiZhaoCard.cs",
    "Cards\WangQuanCard.cs",
    "Cards\DiGuoYuWeiCard.cs",
    "Cards\ChengTianShouMingCard.cs",
    "Cards\TianFengXingTaiCard.cs",
    "Powers\DiGuoYuWeiPower.cs",
    "Powers\TianFengXingTaiPower.cs",
    "Rules\Edict\EdictCombatState.cs",
    "Rules\Edict\EdictPlayedContext.cs",
    "Rules\Edict\IEdictEvents.cs",
    "Rules\Edict\EdictListenerRegistry.cs",
    "Rules\Edict\EdictService.cs"
)
...
$RequiredMarkers = @(
    ...
    "class EdictCombatState",
    "record EdictPlayedContext",
    "interface IAfterEdictPlayed",
    "class EdictService",
    "NotifyAfterPlayed",
    "class EdictCard",
    "class ChuanLingCard",
    "class MiZhaoCard",
    "class WangQuanCard",
    "class DiGuoYuWeiCard",
    "class DiGuoYuWeiPower",
    "class ChengTianShouMingCard",
    "class TianFengXingTaiCard",
    "class TianFengXingTaiPower",
...
foreach ($Marker in $RequiredMarkers) {
    if (-not $SourceText.Contains($Marker)) {
        throw ("R8 source marker missing: {0}" -f $Marker)
    }
}
...
foreach ($Pattern in $ForbiddenRegexPatterns) {
    if ($SourceText -match $Pattern) {
        throw ("Forbidden R8 source pattern found: {0}" -f $Pattern)
    }
}
...
foreach ($Marker in @(
    ...
    if (-not $CombatRules.Contains($Marker)) {
        throw ("CanAoCombatRules R8 marker missing: {0}" -f $Marker)
    }
}
...
$CardKeys = @(
    ...
    "CHUAN_LING_CARD.title",
    "CHUAN_LING_CARD.description",
    "MI_ZHAO_CARD.title",
    "MI_ZHAO_CARD.description",
    "WANG_QUAN_CARD.title",
    "WANG_QUAN_CARD.description",
    "DI_GUO_YU_WEI_CARD.title",
    "DI_GUO_YU_WEI_CARD.description",
    "CHENG_TIAN_SHOU_MING_CARD.title",
    "CHENG_TIAN_SHOU_MING_CARD.description",
    "TIAN_FENG_XING_TAI_CARD.title",
    "TIAN_FENG_XING_TAI_CARD.description"
)
...
$PowerKeys = @(
    "DI_GUO_YU_WEI_POWER.title",
    "DI_GUO_YU_WEI_POWER.description",
    "TIAN_FENG_XING_TAI_POWER.title",
    "TIAN_FENG_XING_TAI_POWER.description"
)
...
if ($Manifest.version -ne "0.0.8") {
    throw ("Unexpected R8 manifest version: {0}" -f $Manifest.version)
}
...
Write-Host "Verified R5/R6/R7 gameplay-core hashes passed."
Write-Host "R8 edict system, UTF-8, manifest and localization verification passed."
