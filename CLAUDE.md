...
当前版本：**R11，基线为用户实机验证通过的 R9 工作区 + R10 角色**。
...
- 探针卡移出玩家可见卡池；
- **R11 补完能力牌**：火刃（衍生）、不堕、凤魂、交辉、守缺、登基、
  凤火军械、终章、瓦魔远征、王座孤明、不灭王朝、万邦来朝
  （`docs/R11_POWER_CARDS.md`，`scripts/Verify-R11.ps1`）；
- **全卡表设计一致性审查与修复**（同文档附录）：修复宴请八方缺消耗、
  辉耀零头格挡、复辟临时凤威泄漏、两件遗物闩锁跨战斗残留、
  旧王复临锁存时机、税契金币通道、33 卡悬浮提示缺失等。
...
`scripts/Verify-R11.ps1` 会验证未被后续阶段有意修改的 R5–R9 核心文件 SHA-256，尤其包括：
